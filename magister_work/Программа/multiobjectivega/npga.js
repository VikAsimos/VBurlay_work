...
for (i = 0; i < length; i += 2) {
	...
	var rand1, rand2, mom, dad, factor, niche = 1;
	
	//турнир для каждой из двух особей проводится со случайной подпопуляцией
	...
	if ((firstwin == 1) && (secoundwin == 0)) mom = rand1;
	else if ((secoundwin == 1) && (firstwin == 0)) mom = rand2;
	// если нет недоминируемого решения, то выбирается особь с меньшим числом особей в окрестности
	else {
		var nichecount1 = 0, nichecount2 = 0;
		for (j = 0; j < length/10; j++) {
			var distance1 = 0, distance2 = 0;
			distance1 = Math.sqrt( Math.pow(inds[j]-inds[rand1]) + Math.pow(indv[j]-indv[rand1]) + Math.pow(indl[j]-indl[rand1]) + Math.pow(indd[j]-indd[rand1]) + Math.pow(indd0[j]-indd0[rand1]));
			distance2 = Math.sqrt( Math.pow(inds[j]-inds[rand2]) + Math.pow(indv[j]-indv[rand2]) + Math.pow(indl[j]-indl[rand2]) + Math.pow(indd[j]-indd[rand2]) + Math.pow(indd0[j]-indd0[rand2]));
			if (distance1 <= niche) nichecount1++;
			if (distance2 <= niche) nichecount2++;
		}
		if (nichecount1 < nichecount2) mom = rand1; else mom = rand2;
	...
	if ((firstwin == 1) && (secoundwin == 0)) dad = rand1;
	else if ((secoundwin == 1) && (firstwin == 0)) dad = rand2;
	else {
		var nichecount1 = 0, nichecount2 = 0;
		for (j = 0; j < length/10; j++) {
			var distance1 = 0, distance2 = 0;
		...
		if (nichecount1 < nichecount2) dad = rand1; else dad = rand2;
